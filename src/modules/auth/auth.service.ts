...
export class AuthService {
  ...
  async login(res: Response, userCredentials: LoginUserDto): Promise<any> {
    ...
    const payload = { _id: user._id, email: user.email, role: user.type, tokenType: TokenType.AuthToken };
  ...
  async sendEmailOtp(@Req() req: Request){
    ...
    if(!await this.otpService.sendOtp(user.email, OtpType.emailverification)){
      throw new HttpException('could not send otp! Please try again after some time', HttpStatus.INTERNAL_SERVER_ERROR); 
    }
  ...
  async sendPhoneOtp(@Req() req: Request){
    ...
    if(!await this.otpService.sendSMS(user.phone, OtpType.phoneverification)){
      throw new HttpException('could not send otp! Please try again after some time', HttpStatus.INTERNAL_SERVER_ERROR); 
    }
    return `Otp sent successfully`;
  }
  ...
  async verifyEmailOtp(@Req() req: Request, @Body() body: VerifyOtpDto){
    ...
    await this.otpService.verifyOtp(body.otp, user.email, OtpType.emailverification);
    await this.User.findOneAndUpdate({email: email},{isEmailVerified: true});
  ...
  async verifyPhoneOtp(@Req() req: Request, @Body() body: VerifyOtpDto){
    ...
    await this.otpService.verifyOtp(body.otp, user.phone, OtpType.phoneverification);
    await this.User.findOneAndUpdate({phone: phone},{isPhoneVerified: true});
  ...
  async forgotPassword(
    ...
    const payload = { _id: user._id, email: user.email, tokenType: TokenType.ResetToken };
  ...
  async resetPassword(
    ...
    await this.otpService.verifyOtp(body.otp, user.email, OtpType.forgotPassword);
